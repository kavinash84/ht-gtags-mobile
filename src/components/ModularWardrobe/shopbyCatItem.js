...
class ShopbyCatItem extends Component {
  getDetails = (component, data) => {
    switch (component) {
      ...
      case 7:
        return (
          <Div className={`${styles.sliderItem}`} style={{ padding: '15px' }}>
        ...
        return (
          <Div className={`${styles.finacesliderItem}`}>
            <Div style={{ backgroundColor: 'white', padding: '20px', textAlign: 'left' }}>
              {/* <Link className={styles.link} to={data.url_key}> */}
              <div className={styles.link}>
                <img data-src={data.imgSrc} src={`${data.imgSrc}?blur=30`} alt={data.title} style={{ marginTop: '15px' }} />
          ...
          <Div className={`${styles.sliderItem}`}>
            ...
            <div className={styles.link}>
              <div style={{ height: '105px' }}>
                <img data-src={data.imgSrc} src={`${data.imgSrc}?blur=30`} alt={data.title} style={{ height: '100%', width: 'auto', margin: 'auto' }} />
              </div>
        ...
        return (
          <Div className={`${styles.sliderItem}`}>
            <Link
              ...
              to={data.url_key}
              onClick={() => {
                sessionStorage.setItem('MWscrollPosition', window.pageYOffset);
              }}
            >
              {/* <div
              className={styles.link}
  ...
  handleScrollPosition = () => {
    const scrollPosition = sessionStorage.getItem('MWscrollPosition');
    if (scrollPosition) {
      window.scrollTo(0, parseInt(scrollPosition));
      setTimeout(function() {
        sessionStorage.removeItem('MWscrollPosition');
      }, 2000);
    }
  };
