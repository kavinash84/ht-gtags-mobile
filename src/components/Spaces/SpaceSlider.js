...
class SpaceSlider extends Component {
  render() {
    const { data, triggerSlideChange, triggerSlideClick, reference, newSettings, mb } = this.props;
      ...
      >
        {data.map((slide, index) => (
          <div key={String(index)}>
            <SpaceItems
              target={slide.target || ''}
              image={slide.m_image || slide.imgSrc}
              url={slide.url_key}
              title={slide.title || ''}
...
export default connect(null, { triggerSlideChange: triggerImpression, triggerSlideClick: triggerClick })(
  SpaceSlider
);