...
const onClick = (key, skuId, simpleSku, session, pincode, configId) => dispatcher => e => {
  e.preventDefault();
  dispatcher(key, skuId, simpleSku, session, pincode, configId);
};

const mapStateToProps = ({
  app, pincode, cart, cart: { addingToCart, addedToCart, key }
}) => ({
            ...
            {!checkStatus ? (
              <Div>
                <Button
                  ...
                  lh="1.5"
                  disabled={addLoading}
                  onClick={onClick(itemId, sku, simpleSku, session, pincode, configId)(addToCart)}
                >
                  {!addLoading && (
                    <AddCart
                      width="18px"
              ...
              <Div>
                <Link className={`${styles.goToCart} ${height !== 'auto' && styles.heightFix} `} to={CART_URL}>
                  ...
                  <Span ml="0.625rem" fontSize="12px" fontFamily="regular" color="#FFF" va="text-bottom" lh="1.5">
                    Go to Cart
                  </Span>
                </Link>
              </Div>
